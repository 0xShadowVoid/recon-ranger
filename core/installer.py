...
class ReconRangerInstaller:
    ...
    def install_category(self, category: str) -> bool:
        ...
        if category not in CATEGORIES:
            ...
            print(
                f"{Colors.CYAN}Available categories: {', '.join(CATEGORIES.keys())}{Colors.RESET}"
            )
            return False

        tools_to_install = CATEGORIES[category]
        print(
            f"{Colors.BLUE}📂 Installing {category} category ({len(tools_to_install)} tools){Colors.RESET}"
        )
    ...
    def install_multiple_tools(self, numbers: List[int]) -> bool:
        ...
        for num in numbers:
            if 1 <= num <= len(tool_list):
                ...
                print(
                    f"{Colors.YELLOW}⚠️ Tool #{num} not found (1-{len(tool_list)}){Colors.RESET}"
                )
        ...
        print(
            f"{Colors.BLUE}🔧 Installing {len(tools_to_install)} tools by number{Colors.RESET}"
        )
        return self._install_tools_list(tools_to_install)
    ...
    def install_range(self, start: int, end: int, skip: List[int] = None) -> bool:
        ...
        if start < 1 or end > len(tool_list) or start > end:
            print(
                f"{Colors.RED}❌ Invalid range: {start}-{end} (1-{len(tool_list)}){Colors.RESET}"
            )
            return False
        ...
        if not tools_to_install:
            print(
                f"{Colors.RED}❌ No tools to install in range {start}-{end}{Colors.RESET}"
            )
            return False

        skip_str = f" (skipping {skip})" if skip else ""
        print(
            f"{Colors.BLUE}🔧 Installing range {start}-{end}{skip_str}: {len(tools_to_install)} tools{Colors.RESET}"
        )
        return self._install_tools_list(tools_to_install)
    ...
    def _install_tools_list(self, tools_to_install: List[str]) -> bool:
        ...
        with tqdm(
            total=len(tools_to_install), desc="Installing tools", unit="tool"
        ) as pbar:
            for tool_name in tools_to_install:
                if tool_name not in self.tools:
                    print(
                        f"{Colors.YELLOW}⚠️ Tool '{tool_name}' not found in configuration{Colors.RESET}"
                    )
    ...
    def rollback_tool(self, tool_name: str) -> bool:
        ...
        try:
            ...
            print(
                f"{Colors.GREEN}✅ Rolled back {binary_name} to backup from {latest_backup.name}{Colors.RESET}"
            )
            return True
        except Exception as e:
    ...
    def install_python(self, name: str, cfg: dict) -> bool:
        ...
        try:
            result = self._run(
                [py, "-c", f"import {binary}; print({binary}.__file__)"], timeout=30
            )
            if result[0]:  # Success
    ...
    def install_git(self, name: str, cfg: dict) -> bool:
        ...
        if path.exists():
            ...
            result = subprocess.run(
                ["git", "clone", "--depth", "1", cfg["repo"], str(path)],
                timeout=180,
                env=env,
                capture_output=True,
                text=True,
        ...
        if launcher_created:
            ...
            error_logger.error(
                f"Could not create launcher for {name}, binary not found in {path}"
            )
            return False
    ...
    def run(self, args):
            ...
            if choice == "all":
                ...
                try:
                    ...
                    if idx < 0 or idx >= len(self.tools):
                        print(
                            f"{Colors.RED}Invalid selection. Must be between 1 and {len(self.tools)}{Colors.RESET}"
                        )
                        sys.exit(1)
                    targets = [list(self.tools.keys())[idx]]
                except ValueError:
                    print(
                        f"{Colors.RED}Invalid input. Please enter a number or 'all'{Colors.RESET}"
                    )
                    sys.exit(1)
        ...
        if skipped:
            print(
                f"{Colors.CYAN}⊘ Skipped (already installed): {len(skipped)}{Colors.RESET}"
            )
        if failed:
